...
export default ({ server, className }: { server: Server; className?: string }) => {
    ...
    }, [isSuspended, getStats, then, interval, current, setInterval, clearInterval]);

    const alarms = { cpu: false, memory: false, disk: false };
    if (stats) {
        alarms.cpu =
            server.limits.cpu === 0 ? false : stats.cpuUsagePercent >= server.limits.cpu * 0.9;
    ...
    const diskLimit =
        server.limits.disk !== 0 ? bytesToString(mbToBytes(server.limits.disk)) : 'Unlimited';
    const memoryLimit =
        server.limits.memory !== 0 ? bytesToString(mbToBytes(server.limits.memory)) : 'Unlimited';
